\todo{Switch out 'completeness' for 'loosing parts of our search space'?}
\todo{replace expanding bloom filters with scalable bloom filters as the correct name}
...
\todo{We are most interested in finding nodes that can be reached in many ways (on different PEs). Pure recursion can be rather 'easily' detected locally}
...
We will now take a closer look at the probabilities involved. Assuming we use an expanding bloom filter with maximum false positive rate $p < 1$ and a hash function which maps search nodes to uniformly independent values and $n$ is a shortest path. Then
\[
q = 1 - (1 - p)^l < 1
\]
...
\end{comment}
To integrate bloom filters into our distributed loop detection, we also need to address the question of what to do in case the global filter gets filled up, i.e. it's false positive rate reaches our set limit. For local loop detection we introduced expanding bloom filters.
This is a problem as we now communicate whole sets of search nodes whereas locally we introduce new search nodes into the filter one by one, increasing the size at exactly the right moment. As a result, we face the choice of increasing the size early, throwing away some information or loosing our guarantees regarding false positives. Similarly, we face the problem where different PEs may disagree about the current maximum size of the expanding bloom filter, putting more information in a smaller filter that will be thrown away by other PEs. \\
